		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD NANAMI BOT*',
					...
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman nanami bot Daftar Untuk Berteman Dengan nanami bot dengam cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar nanami|17\n\n──「 NANAMI BOT 」──`,
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT NANAMI✨* 」\n*│+wa.me/62895704959080*╰──────*「 *NANAMI* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  NANAMI BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/62895704959080*╰──────*「 *NANAMI* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  NANAMI BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
		...
		case 'iri':
            ...
            break
                ...
                if (!isPublic) return reply(mess.only.public)
					...
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar NANAMI|12`)
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('github.com/Nanamibot/Nanami-BOT@s.whatsapp.net', options, text, {quoted: mek})
 ...
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./src/NANAMIjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                client.sendMessage(hasil, image, mek, '\`\`\`NANAMIJOKES\`\`\`')
				break
					case 'kata':
					if (!isPublic) return reply(mess.only.public)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  NANAMI BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * NANAMI Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner NANAMI!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin NANAMI!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
